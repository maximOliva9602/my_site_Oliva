...
(function () {
  ...
  function boot(me) {
    ...
    var TAB_ICOS  = { dashboard:"📊", podii:"📌", zapysy:"📋", rozklad:"📅", grafik:"🗓", clients:"👤", analytics:"📈", traffic:"🌐", reviews:"⭐", services:"💆", masters:"👥", users:"🔐", notif:"🔔", broadcast:"📣", filiyi:"🏢", hero:"🖼", certs:"🎁" };
  ...
  function renderAppts(opts) {
    ...
    if (ME.role === "owner") {
      Promise.all([api("GET", "/api/crm/masters"), api("GET", "/api/crm/branches")]).then(function (res) {
        ...
        masterFilterWrap.appendChild(masterSel);
    ...
    function loadCalendar(masterFilter, opts) {
      ...
      Promise.all([
        ...
        var apptById = {};
        appts.forEach(function(a) { apptById[a.id] = a; });
            ...
            if (heightPx >= 22) {
              html += '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:2px;margin-bottom:1px;">' +
                ...
                '<span style="display:flex;gap:3px;align-items:center;flex-shrink:0;line-height:1;">' +
                  (a.is_new_client ? '<span title="Це був перший візит клієнта" style="font-size:.56rem;font-weight:800;color:#1a3d0f;background:#d9ff9f;border-radius:6px;padding:1px 4px;letter-spacing:.02em;">NEW</span>' : '') +
  ...
  function apptModal(opts) {
    ...
    /* CRM-категорія → ключ групи "Новий запис". Ті самі канонічні значення,
       що власник обирає в адмінці («Розділ на сторінці запису»). */
    var SVC_CAT_TO_GROUP = {
      "загальні масажі": "massage", "прайс майстер": "massage", "прайс топ майстер": "massage", "прайс експерт": "massage",
      "spa-масажі для двох": "spa-two", "spa-масажі для одного": "spa-one", "spa-ритуали": "spa-one",
      "комплекси": "body", "корекція фігури": "body",
      "додаткові послуги": "extra",
    };
    function serviceGroup(s) {
      var name = String((s && s.name) || "").toLowerCase();
      var cat = String((s && s.category) || "").trim().toLowerCase();
      /* Явне «для двох» / «для одного» в назві — визначає групу однозначно,
         незалежно від категорії: саме назву бачить клієнт. */
      if (/парний|чотири руки|для двох/.test(name)) return "spa-two";
      if (/для одного/.test(name)) return "spa-one";
      /* Категорія, яку власник обрав в адмінці, — пріоритет над евристикою
         за назвою нижче. Без цього кроку, наприклад, «Антицелюлітний
         масаж» завжди йшов у «Корекцію фігури», навіть якщо власник
         свідомо переніс його в «Масажі» — вибір мовчки ігнорувався. */
      if (SVC_CAT_TO_GROUP[cat]) return SVC_CAT_TO_GROUP[cat];
  ...
  function renderNotif() {
    ...
    var cbConfirm = togRow("Підтвердження запису",
      ...
      "Ваш запис {дата} о {час} підтверджений. До зустрічі!\n{телефон студії}",
    ...
    var cbResched = togRow("Перенесення візиту",
      ...
      "Ваш візит перенесено на {дата} о {час}. Чекаємо!\n{телефон студії}",
