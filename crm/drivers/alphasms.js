...
module.exports = {
  ...
  async sendMessage(opts) {
    ...
    // Не палимо гроші на завідомо некоректних номерах — AlphaSMS ТАРИФІКУЄ
    // спробу навіть при «Помилка в номері» (27.07: биті тестові номери
    // з'їдали по 3.19 грн). Тому для укр. номерів — строго: 380 + код
    // мобільного оператора + 7 цифр (12 разом). Стаціонарні (38044, 38048…)
    // і номери з пропущеною цифрою відсікаються ще до звернення до API.
    // Іноземні (не 380…) — базова перевірка довжини 11–15 цифр.
    const UA_MOBILE = /^380(39|50|63|66|67|68|73|75|77|89|9[1-9])\d{7}$/;
    const ok = phone.startsWith("380") ? UA_MOBILE.test(phone) : /^\d{11,15}$/.test(phone);
    if (!ok) {
      throw new Error("alphasms: некоректний номер «" + opts.phone + "» — пропущено без списання (для України очікується мобільний 380XXXXXXXXX)");
    }
    const id = nextId();
    const j = await call([{
      type: "sms",
