...
/* Перерва на дату — у філії тієї колонки розкладу, де її поставили. Майстер
   може того дня бути в кількох філіях (окремі колонки), і перерва в Успішній
   не має закривати час у Борщагівській. NULL — старі перерви й колонка без
   поділу на філії: діють у всіх філіях майстра, як і раніше. */
try { db.exec("ALTER TABLE day_blocks ADD COLUMN branch_id INTEGER"); } catch (e) {}
