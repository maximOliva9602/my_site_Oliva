...
/* Всі активні майстри з фото та рівнем. show_on_site НЕ фільтруємо тут —
   він ховає майстра лише з лендінгу (команда на головній, server.js
   /api/masters), а онлайн-запис і далі має пропонувати його клієнтам. */
...
router.get("/branches", function (req, res) {
  ...
  const branches = db.prepare(
    "SELECT id, name, photo, address, subtitle, nearby, photos FROM branches WHERE active=1 ORDER BY sort_order, id"
  ).all();
  ...
  branches.forEach(function (b) {
    ...
    /* Галерея: JSON-масив; у старих філій його немає — тоді єдине photo. */
    let list = [];
    if (b.photos) { try { const a = JSON.parse(b.photos); if (Array.isArray(a)) list = a; } catch (e) {} }
    if (!list.length && b.photo) list = [b.photo];
    b.photos = list;
  });
